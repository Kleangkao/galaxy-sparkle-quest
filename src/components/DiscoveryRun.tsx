...
export default function DiscoveryRun({ gameState, onBack, onComplete }: Props) {
  ...
  const decoyId = nextRequiredId === undefined
    ? undefined
    : points.find((item) =>
      item.id < requiredFinds &&
      item.id !== nextRequiredId &&
  ...
  const find = (item: (typeof points)[number]) => {
    ...
    if (isRequired && item.id !== nextRequiredId) {
      setWrongPick(item.id);
      return;
    }
    if (!isRequired && !complete) return;
  ...
  const nextPoint = nextRequiredId === undefined ? undefined : points[nextRequiredId];
  const cluePosition = nextPoint
    ? tr(
      `${nextPoint.y < 42 ? "Upper" : nextPoint.y > 64 ? "Lower" : "Middle"} ${nextPoint.x < 40 ? "left" : nextPoint.x > 68 ? "right" : "area"}`,
      `${nextPoint.y < 42 ? "ด้านบน" : nextPoint.y > 64 ? "ด้านล่าง" : "ช่วงกลาง"}${nextPoint.x < 40 ? "ซ้าย" : nextPoint.x > 68 ? "ขวา" : ""}`,
    )
    : "";
  ...
  return (
    <main className="discovery-mode relative z-10 mx-auto min-h-screen max-w-7xl px-5 pb-28 pt-24 lg:px-8">
      <header className="discovery-header"><button onClick={() => setBiome(null)}><ArrowLeft className="h-4 w-4" /> {tr("Biomes", "พื้นที่สำรวจ")}</button><div><div className="command-kicker">{tr(`Discovery Run · ${biome.name}`, `ออกสำรวจ · ${biome.nameTh}`)}</div><h1>{tr(`Follow the signal trail${rareSignalActive ? " and find the rare anomaly" : ""}.`, `ตามรอยสัญญาณ${rareSignalActive ? " และค้นหาสัญญาณหายาก" : ""}`)}</h1><p>{tr("Read the location clue, then choose between two possible signals. A wrong guess has no penalty.", "อ่านคำใบ้ตำแหน่ง แล้วเลือกจากสัญญาณที่เป็นไปได้ 2 จุด ตอบผิดไม่เสียอะไร")}</p></div><div className="discovery-pilot"><img src={pilot.image} alt="" /><span>{getMasteryTier(currentMastery, lang)}<small>{found.length}/{points.length} {tr("discoveries", "สิ่งที่พบ")}</small></span></div></header>
      <section className="discovery-run-guide"><div><MousePointerClick className="h-4 w-4" /><span>{tr("Current clue", "เบาะแสตอนนี้")}<strong>{complete ? tr("Trail complete · claim your rewards", "ตามรอยครบแล้ว · รับรางวัลได้เลย") : tr(`Signal ${found.filter((id) => id < requiredFinds).length + 1}/${requiredFinds} · Search the ${cluePosition}`, `สัญญาณ ${found.filter((id) => id < requiredFinds).length + 1}/${requiredFinds} · มองหา${cluePosition}`)}</strong></span></div><div><Gift className="h-4 w-4" /><span>{tr("Journal reward", "รางวัลสมุดบันทึก")}<strong>{tr(`+${Math.ceil(requiredFinds * puri.rewardMultiplier * modifiers.crystalMultiplier)} crystals${rareSignalActive ? ` · rare signal +${Math.ceil(puri.rewardMultiplier * modifiers.crystalMultiplier)}` : ""}`, `+${Math.ceil(requiredFinds * puri.rewardMultiplier * modifiers.crystalMultiplier)} คริสตัล${rareSignalActive ? ` · สัญญาณหายาก +${Math.ceil(puri.rewardMultiplier * modifiers.crystalMultiplier)}` : ""}`)}</strong></span></div><button onClick={scan} disabled={scanCharges <= 0 || scanActive}><ScanLine className="h-4 w-4" /> {tr(`Reveal answer · ${scanCharges} left`, `เฉลยจุด · เหลือ ${scanCharges} ครั้ง`)}</button></section>
      <section className="discovery-layout">
        <div className="discovery-scene"><img className="discovery-scene__backdrop" src={biome.backdrop} alt={tr(`${biome.name} landscape`, `พื้นที่ ${biome.nameTh}`)} /><div className="discovery-scene__wash" /><div className="discovery-scene__hint"><Leaf className="h-4 w-4" /> {wrongPick !== null ? tr("That signal does not match the clue. Try the other visible point.", "จุดนั้นไม่ตรงกับคำใบ้ ลองอีกจุดที่มองเห็นได้") : puri.discoveryHint ? tr("PURI is highlighting the correct signal.", "PURI กำลังช่วยไฮไลต์สัญญาณที่ถูกต้อง") : tr("Use the location clue above. Two signal points are visible.", "ดูคำใบ้ตำแหน่งด้านบน จะมีสัญญาณให้เลือก 2 จุด")}</div>
          ...
          {points.map((item) => { const isFound = found.includes(item.id); const isRare = item.id >= requiredFinds; const isCandidate = item.id === nextRequiredId || item.id === decoyId; const isLockedClue = !isFound && ((!isRare && !isCandidate) || (isRare && !complete)); const itemName = lang === "th" ? item.nameTh : item.name; return <button key={item.id} aria-label={isLockedClue ? tr("Signal locked until the next clue", "สัญญาณยังล็อกอยู่จนกว่าจะถึงคำใบ้ถัดไป") : tr(`Discover ${isRare ? "rare anomaly " : ""}${item.name}`, `สำรวจ${isRare ? "สัญญาณหายาก " : ""}${item.nameTh}`)} disabled={isLockedClue} onClick={() => find(item)} className={`discovery-point ${isFound ? "is-found" : ""} ${isRare ? "is-rare" : ""} ${isLockedClue ? "is-locked-clue" : ""} ${guidedId === item.id || (scanActive && item.id === nextRequiredId) ? "is-guided" : ""} ${wrongPick === item.id ? "is-wrong" : ""}`} style={{ left: `${item.x}%`, top: `${item.y}%` }} title={isFound ? itemName : undefined}>{isFound ? item.icon : isRare ? "✦" : "?"}</button>; })}
        ...
        <aside className="discovery-journal"><div className="discovery-journal__title"><BookOpen className="h-5 w-5" /><div><span>{tr("Field journal", "สมุดบันทึก")}</span><strong>{biomeName} · {tr(`Run ${runNumber + 1}`, `รอบ ${runNumber + 1}`)}</strong></div></div>
          ...
          <div className="discovery-journal__list">{points.map((item) => <div key={item.id} className={found.includes(item.id) ? "is-found" : ""}><span>{found.includes(item.id) ? item.icon : "?"}</span><strong>{found.includes(item.id) ? (lang === "th" ? item.nameTh : item.name) : tr("Undiscovered", "ยังไม่พบ")}</strong></div>)}</div>
