import { ArrowLeft, ArrowRight, CheckCircle2, Gem, Map, Medal, PawPrint, Shield, Sparkles, Swords, Target, Trophy, Users, Zap } from "lucide-react";
    ...
    <main className="captain-progress relative z-10 mx-auto min-h-screen max-w-7xl px-5 pb-28 pt-28 lg:px-8">
      ...
      <div className="captain-progress__grid">
        ...
        <section className="progress-panel progress-panel--mastery">
          <div className="progress-panel__heading"><Trophy /><div><span>{tr("Three core modes", "3 โหมดหลัก")}</span><h2>{tr("Activity mastery", "ความชำนาญแต่ละโหมด")}</h2></div></div>
        ...
        <section className="progress-panel">
          <div className="progress-panel__heading"><Zap /><div><span>{tr("Action modes strengthen Story", "โหมดต่อสู้ช่วยเนื้อเรื่อง")}</span><h2>{tr("Campaign support", "พลังช่วยเนื้อเรื่อง")}</h2></div><b>{supportPerks.filter((perk) => perk.earned).length}/{supportPerks.length}</b></div>
          <div className="captain-medals">
