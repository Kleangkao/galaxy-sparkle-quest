...
type Enemy = Point & { id: number; hp: number; maxHp: number; speed: number; size: number; kind: EnemyKind; timer: number; phase: number; pulseHit?: number };
...
interface ArenaState {
  ...
  invulnerable: number; pulseCooldown: number; pulseEffect: number; pulseHits: number; pulseHazardsCleared: number; bossSpawned: boolean; bossDefeated: boolean; bossWarning: number; bossIntro: number;
...
const makeArena = (bonusHull = 0): ArenaState => ({
  ...
  pulseEffect: 0, pulseHits: 0, pulseHazardsCleared: 0,
...
export default function SwarmProtocol({ gameState, suspended = false, onActiveChange, onBack, onComplete }: Props) {
  ...
  const objectiveText = tr(`Build perks and defeat Ahr · ${runVariant.name}`, `เลือกพลังระหว่างทาง แล้วกำจัด Ahr ใน${runVariant.nameTh}`);
  ...
  const activatePulse = useCallback(() => {
    ...
    const pulseHits = state.enemies.filter((enemy) => distance(enemy, state.player) < radius).length;
    const pulseHazardsCleared = state.hazards.filter((hazard) => distance(hazard, state.player) < radius).length;
    state.enemies = state.enemies.map((enemy) => distance(enemy, state.player) < radius ? { ...enemy, hp: enemy.hp - 45, pulseHit: 0.45 } : enemy);
    ...
    state.pulseEffect = 0.7;
    state.pulseHits = pulseHits;
    state.pulseHazardsCleared = pulseHazardsCleared;
    setFrame({ ...state, enemies: [...state.enemies], hazards: [...state.hazards], player: { ...state.player } });
    playImpactSound();
    pulseGamepad(110, pulseHits || pulseHazardsCleared ? 0.65 : 0.3);
  ...
  useEffect(() => {
    ...
    const timer = window.setInterval(() => {
      ...
      const state = arena.current;
      state.elapsed += dt; state.fireTimer -= dt; state.spawnTimer -= dt; state.invulnerable -= dt; state.pulseCooldown -= dt; state.pulseEffect -= dt; state.bossWarning -= dt; state.bossIntro -= dt; state.bossAttackTimer -= dt;
      state.enemies.forEach((enemy) => { enemy.pulseHit = Math.max(0, (enemy.pulseHit ?? 0) - dt); });
  ...
  const bossStatus = frame.bossSpawned ? tr("Boss active", "บอสมาแล้ว") : tr(`Boss in ${Math.max(0, Math.ceil(bossTime - frame.elapsed))}s`, `บอสจะมาใน ${Math.max(0, Math.ceil(bossTime - frame.elapsed))} วิ`);
  ...
  const perkChoices = [
    {
      ...
      pair: tr("Pair with Rapid Fire → Overdrive", "ใช้คู่กับยิงถี่เพื่อปลดล็อกโอเวอร์ไดรฟ์"),
    },
    {
      kind: "fireRate" as const,
      name: tr("Rapid Fire", "ยิงถี่"),
      effect: tr("Fire 20% faster", "ยิงเร็วขึ้น 20%"),
      pair: tr("Pair with Heavy Shots → Overdrive", "ใช้คู่กับกระสุนหนักเพื่อปลดล็อกโอเวอร์ไดรฟ์"),
    },
    {
      kind: "speed" as const,
      name: tr("Thrusters", "เครื่องยนต์เร่ง"),
      effect: tr("Move 22% faster", "เคลื่อนที่เร็วขึ้น 22%"),
      pair: tr("Pair with Energy Pull → Phase Drive", "ใช้คู่กับแรงดูดพลังเพื่อปลดล็อกเฟสไดรฟ์"),
    },
    {
      kind: "magnet" as const,
      name: tr("Energy Pull", "แรงดูดพลัง"),
      effect: tr("Pickup radius +22%", "ระยะเก็บพลังเพิ่ม 22%"),
      pair: tr("Pair with Thrusters → Phase Drive", "ใช้คู่กับเครื่องยนต์เร่งเพื่อปลดล็อกเฟสไดรฟ์"),
    },
    {
      kind: "pulse" as const,
      name: tr("Pulse Recharge", "ชาร์จคลื่นไว"),
      effect: tr("Pulse cooldown 9s → 7.4s", "รอใช้คลื่นจาก 9 วิ เหลือ 7.4 วิ"),
      pair: tr("Pair with Field Repair → Guardian Core", "ใช้คู่กับซ่อมฉุกเฉินเพื่อปลดล็อกแกนพิทักษ์"),
    },
    {
      kind: "repair" as const,
      name: tr("Field Repair", "ซ่อมฉุกเฉิน"),
      effect: tr("Recover 32 hull now", "ฟื้นพลังยาน 32 หน่วยทันที"),
      pair: tr("Pair with Pulse Recharge → Guardian Core", "ใช้คู่กับชาร์จคลื่นไวเพื่อปลดล็อกแกนพิทักษ์"),
    },
  ];
  ...
  return <main className={`combat-mode relative z-10 mx-auto min-h-screen max-w-7xl px-5 pb-28 pt-28 lg:px-8 ${running || ended ? "is-active" : ""} ${gameState.accessibility.effects === "reduced" ? "effects-reduced" : ""}`}>
    ...
    <header className="combat-header"><button onClick={onBack}><ArrowLeft className="h-4 w-4" /> {tr("Modes", "กลับไปเลือกโหมด")}</button><div><span>{tr("Swarm Protocol · Survival", "ฝ่าฝูงศัตรู · เอาตัวรอด")}</span><strong>{tr("AHR INCURSION", "AHR บุกโจมตี")}</strong></div><div className="combat-header__loadout"><span>{pilot.name}</span><span>{tool.name}</span></div></header>
    ...
    <div className="combat-arena-wrap"><div className={`combat-arena ${frame.invulnerable > 0 ? "is-hit" : ""} ${frame.bossWarning > 0 ? "boss-warning" : ""} ${frame.bossIntro > 0 ? "boss-arrival" : ""}`} style={{ aspectRatio: `${WIDTH}/${HEIGHT}` }}><div className="combat-grid" />
      ...
      {frame.bossIntro > 0 && <div className="boss-message is-arrival">{tr("AHR CORE ENTERING · FOCUS FIRE", "แกน AHR เข้ามาแล้ว เล็งยิงที่บอส")}</div>}
      ...
      {frame.enemies.map((enemy) => <span key={enemy.id} className={`combat-enemy is-${enemy.kind} ${(enemy.pulseHit ?? 0) > 0 ? "is-pulse-hit" : ""} ${enemy.kind === "dasher" && enemy.timer < 0.35 && enemy.timer >= 0 ? "is-telegraph" : ""} ${enemy.kind === "boss" && frame.bossWarning > 0 ? "is-casting" : ""}`} style={{ left: `${enemy.x / WIDTH * 100}%`, top: `${enemy.y / HEIGHT * 100}%`, width: enemy.size * 2, height: enemy.size * 2 }}>{enemy.kind === "boss" ? <img src="/assets/galia-current/ahr-boss-master-v3.webp" alt="Ahr boss" /> : <b>{enemy.kind === "dasher" ? "›" : enemy.kind === "orbiter" ? "◎" : enemy.kind === "elite" ? "◆" : ""}</b>}{enemy.kind === "boss" && <i><b style={{ width: `${enemy.hp / enemy.maxHp * 100}%` }} /></i>}</span>)}
      {frame.pulseEffect > 0 && <span className="combat-pulse-wave" style={{ left: `${frame.player.x / WIDTH * 100}%`, top: `${frame.player.y / HEIGHT * 100}%` }} />}
      ...
      {frame.pulseEffect > 0 && <div className="combat-pulse-feedback" role="status" aria-live="polite">{frame.pulseHits || frame.pulseHazardsCleared ? tr(`Shock Pulse hit ${frame.pulseHits} and cleared ${frame.pulseHazardsCleared} hazards`, `คลื่นกระแทกโดนศัตรู ${frame.pulseHits} ตัว และสลายกระสุน ${frame.pulseHazardsCleared} ลูก`) : tr("Shock Pulse fired. No targets nearby.", "ปล่อยคลื่นกระแทกแล้ว แต่ไม่มีเป้าหมายอยู่ใกล้")}</div>}
      {!running && !ended && (
        <ModeStartOverlay
          mode="swarm"
          icon={<Sparkles className="h-7 w-7" />}
          kicker={tr(`${duration}-second survival · ${runVariant.name}`, `${runVariant.nameTh} ใช้เวลา ${duration} วินาที`)}
          ...
          note={tr(`Space / controller A · Shock Pulse deals 45 damage and clears nearby hazards. Rewards count after active movement or ${SWARM_PARTICIPATION.energyCollected} energy.`, `กด Space หรือปุ่ม A บนจอยเพื่อใช้คลื่นกระแทก สร้างความเสียหาย 45 และทำลายกระสุนอันตรายรอบตัว ต้องขยับหลบหรือเก็บพลัง ${SWARM_PARTICIPATION.energyCollected} ชิ้นขึ้นไป รอบนี้จึงจะนับรางวัล`)}
          primaryLabel={tr("Begin run", "เริ่มเล่น")}
          onStart={reset}
        />
      )}
      {upgradeLevel !== null && <div className="combat-overlay"><div className="command-kicker">{tr(`Perk level ${upgradeLevel}`, `เลือกรอบที่ ${upgradeLevel}`)}</div><h2>{tr("Choose one upgrade", "เลือกความสามารถ 1 อย่าง")}</h2><p>{tr("Green text is the immediate effect. The smaller line shows which second perk creates an evolution.", "ข้อความสีเขียวคือผลที่ได้ทันที ส่วนบรรทัดล่างบอกว่าต้องใช้คู่กับความสามารถไหนจึงจะปลดล็อกพลังผสม")}</p><div className="combat-upgrades">{perkChoices.map((perk) => <button key={perk.kind} onClick={() => chooseUpgrade(perk.kind)}><span>{perk.name}</span><strong>{perk.effect}</strong><small>{perk.pair}</small></button>)}</div></div>}
    ...
    <footer className="combat-controls"><span>{combatInput.source === "controller" ? tr("Controller connected · Left stick moves", "เชื่อมต่อจอยแล้ว ใช้อนาล็อกซ้ายเพื่อขยับ") : combatInput.source === "touch" ? tr("Touch controls active", "ใช้ปุ่มบนหน้าจอเพื่อขยับ") : tr("WASD / arrows · Move", "ใช้ WASD หรือปุ่มลูกศรเพื่อขยับ")}</span><button className={`combat-pulse-control ${frame.pulseCooldown > 0 ? "is-cooling" : "is-ready"}`} onClick={activatePulse} disabled={!running || effectivePaused || frame.pulseCooldown > 0} title={tr("Damages nearby enemies and clears nearby hazard shots", "โจมตีศัตรูและสลายกระสุนอันตรายรอบตัว")} aria-keyshortcuts="Space"><Zap className="h-4 w-4" /><span><strong>Space / A · {tr("Shock Pulse", "คลื่นกระแทก")}</strong><small>{frame.pulseCooldown > 0 ? tr(`Charging · ${Math.ceil(frame.pulseCooldown)}s`, `กำลังชาร์จ อีก ${Math.ceil(frame.pulseCooldown)} วิ`) : tr("READY · blast nearby enemies and hazards", "พร้อมใช้ โจมตีศัตรูและสลายกระสุนรอบตัว")}</small></span></button><button onClick={() => setPaused((value) => !value)} disabled={!running || suspended}>{paused ? <Play className="h-4 w-4" /> : <Pause className="h-4 w-4" />}{paused ? tr("Resume", "เล่นต่อ") : tr("Pause", "หยุด")}</button></footer>
