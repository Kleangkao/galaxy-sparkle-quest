...
export default function SwarmProtocol({ gameState, onBack, onOpenHangar, onComplete }: Props) {
  ...
  return <main className={`combat-mode relative z-10 mx-auto min-h-screen max-w-7xl px-5 pb-28 pt-28 lg:px-8 ${gameState.accessibility.effects === "reduced" ? "effects-reduced" : ""}`}>
    ...
    <section className="swarm-purpose"><span><Sparkles className="h-4 w-4" /> {tr("Collect energy to pause and choose run perks.", "เก็บพลังเพื่อหยุดเกมและเลือกความสามารถ")}</span><span><Zap className="h-4 w-4" /> {tr("Every run earns crystals, XP, and PURI bond.", "เล่นทุกรอบได้คริสตัล XP และความสนิทกับ PURI")}</span><button onClick={onOpenHangar}>{tr("Permanent upgrades · Crew Hangar", "อัปเกรดถาวร · จัดทีม")}</button></section>
