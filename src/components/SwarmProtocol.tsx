...
interface Props {
  gameState: GameState;
  variant?: "swarm" | "arcade";
  contractId?: string;
  onBack: () => void;
  onComplete: (result: { score: number; crystals: number; xp: number; won: boolean; variant: "swarm" | "arcade"; contractId?: string }) => void;
}
...
export default function SwarmProtocol({ gameState, variant = "swarm", contractId, onBack, onComplete }: Props) {
  ...
  const duration = (variant === "arcade" ? contract.duration : 75) + modifiers.missionTimeBonus;
  const bossTime = variant === "arcade" ? contract.bossTime : 50;
  const objective = variant === "arcade" ? contract.objective : "boss";
  const objectiveText = objective === "energy" ? `${contract.target} energy` : objective === "score" ? `${contract.target.toLocaleString()} points` : "Defeat Ahr";
  ...
  const finish = useCallback((success: boolean) => {
    ...
    const crystals = Math.ceil(Math.max(2, Math.floor(state.score / (variant === "arcade" ? 280 : 350)) + (success ? 8 : 2)) * puri.rewardMultiplier);
    const xp = Math.max(3, Math.floor(state.elapsed / 6) + (success ? 12 : 3));
    onComplete({ score: state.score, crystals, xp, won: success, variant, contractId: variant === "arcade" ? contract.id : undefined });
  }, [contract.id, ended, onComplete, puri.rewardMultiplier, variant]);

  const activatePulse = useCallback(() => {
    const state = arena.current;
    if (!running || paused || state.pulseCooldown > 0) return;
    const radius = (variant === "arcade" ? 170 : 130) + (gameState.accessibility.aimHelp === "wide" ? 25 : 0);
  ...
  }, [gameState.accessibility.aimHelp, paused, running, variant]);
  ...
  useEffect(() => {
    ...
    const timer = window.setInterval(() => {
      ...
      const spawnMultiplier = variant === "arcade" ? contract.spawnMultiplier : 1;
      const spawnRate = Math.max(0.2, ((variant === "arcade" ? 0.7 : 0.9) - state.elapsed * 0.007) / spawnMultiplier);
      if (state.spawnTimer <= 0) {
        ...
        const hp = (18 + Math.floor(state.elapsed / 12) * 5) * (kind === "orbiter" ? 1.45 : kind === "dasher" ? 0.85 : 1);
        state.enemies.push({ id: state.nextId++, x, y, hp, maxHp: hp, speed: kind === "dasher" ? 58 : kind === "orbiter" ? 48 : 42 + state.elapsed * 0.45, size: kind === "orbiter" ? 15 : 12, kind, timer: 1.6 + Math.random() * 1.5, phase: Math.random() * Math.PI * 2 });
        state.spawnTimer = spawnRate;
      }

      if (bossTime !== null && !state.bossSpawned && state.elapsed >= bossTime) { state.bossSpawned = true; state.enemies.push({ id: state.nextId++, x: WIDTH / 2, y: -55, hp: variant === "arcade" ? 420 : 650, maxHp: variant === "arcade" ? 420 : 650, speed: 28, size: 42, kind: "boss", timer: 4, phase: 0 }); }
      ...
      if (state.invulnerable <= 0 && (enemyHit || hazardHit)) { state.hp -= hazardHit ? 16 : 12; state.invulnerable = 0.8; }
      ...
      const thresholds = [0, 7, 18, 34, 55, 80]; const nextLevel = thresholds.reduce((level, threshold, index) => state.energy >= threshold ? index + 1 : level, 1); if (nextLevel > state.level) { state.level = nextLevel; setUpgradeLevel(nextLevel); }
      setFrame({ ...state, enemies: [...state.enemies], shots: [...state.shots], hazards: [...state.hazards], drops: [...state.drops], player: { ...state.player } });
      const success = objective === "boss" ? state.bossDefeated : objective === "energy" ? state.energy >= contract.target : state.score >= contract.target;
      if (state.hp <= 0) finish(false); else if (success) finish(true); else if (state.elapsed >= duration) finish(false);
    }, tickMs);
    return () => window.clearInterval(timer);
  }, [aimBonus, bossTime, contract.spawnMultiplier, contract.target, duration, finish, gameState.accessibility.combatSpeed, inputVector, objective, paused, puri.combatMagnet, running, upgradeLevel, variant]);

  const chooseUpgrade = (kind: "damage" | "speed" | "fireRate" | "magnet") => { upgrades.current[kind] *= kind === "damage" ? 1.3 : kind === "fireRate" ? 1.2 : 1.22; setUpgradeLevel(null); };
  const boss = frame.enemies.find((enemy) => enemy.kind === "boss");
  const resultReward = useMemo(() => ({ crystals: Math.ceil(Math.max(2, Math.floor(frame.score / (variant === "arcade" ? 280 : 350)) + (won ? 8 : 2)) * puri.rewardMultiplier), xp: Math.max(3, Math.floor(frame.elapsed / 6) + (won ? 12 : 3)) }), [frame.elapsed, frame.score, puri.rewardMultiplier, variant, won]);
  const bossStatus = bossTime === null ? objectiveText : frame.bossSpawned ? "Boss active" : `Boss in ${Math.max(0, Math.ceil(bossTime - frame.elapsed))}s`;

  return <main className={`combat-mode relative z-10 mx-auto min-h-screen max-w-7xl px-5 pb-28 pt-24 lg:px-8 ${gameState.accessibility.effects === "reduced" ? "effects-reduced" : ""}`}>
    <header className="combat-header"><button onClick={onBack}><ArrowLeft className="h-4 w-4" /> {variant === "arcade" ? "Assignments" : "Modes"}</button><div><span>{variant === "arcade" ? "Arcade Ops" : "Swarm Protocol"}</span><strong>{variant === "arcade" ? contract.name.toUpperCase() : "AHR INCURSION"}</strong></div><div className="combat-header__loadout"><span>{pilot.name}</span><span>{tool.name}</span></div></header>
    ...
    <div className="combat-arena-wrap"><div className={`combat-arena ${frame.invulnerable > 0 ? "is-hit" : ""} ${frame.bossWarning > 0 ? "boss-warning" : ""}`} style={{ aspectRatio: `${WIDTH}/${HEIGHT}` }}><div className="combat-grid" />
      ...
      {!running && !ended && <div className="combat-overlay"><div className="command-kicker">Live training simulation</div><h1>{variant === "arcade" ? contract.name : "Swarm Protocol"}</h1><p>{variant === "arcade" ? contract.briefing : "Move, auto-target threats, collect energy, and defeat Ahr."} WASD or arrows move. Space activates your pulse.</p><button onClick={reset}><Play className="h-4 w-4" /> Begin run</button></div>}
      {upgradeLevel !== null && <div className="combat-overlay"><div className="command-kicker">Level {upgradeLevel} reached</div><h2>Choose one upgrade</h2><div className="combat-upgrades"><button onClick={() => chooseUpgrade("damage")}>⚡ Power<strong>+30% damage</strong></button><button onClick={() => chooseUpgrade("fireRate")}>🎯 Rapid<strong>+20% fire rate</strong></button><button onClick={() => chooseUpgrade("speed")}>💨 Boost<strong>+22% speed</strong></button><button onClick={() => chooseUpgrade("magnet")}>🧲 Pull<strong>+22% pickup range</strong></button></div></div>}
      {ended && <div className="combat-overlay"><div className="command-kicker">Simulation complete</div><h2>{won ? "Objective complete!" : frame.hp <= 0 ? "Pilot extracted" : "Objective missed"}</h2><p>Score {frame.score.toLocaleString()} · +{resultReward.crystals} crystals · +{resultReward.xp} XP · PURI bond +{won ? 3 : 1}</p><div className="combat-actions"><button onClick={reset}><Play className="h-4 w-4" /> Run again</button><button onClick={onBack}>{variant === "arcade" ? "Choose assignment" : "Return to modes"}</button></div></div>}
    </div></div>
